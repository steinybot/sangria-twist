...
class CharacterRepo {
  ...
  def getHero(episode: Option[Episode.Value]): Try[Character] = {
    val hero = episode flatMap (_ => getHuman("1000")) orElse[Character] droids.lastOption
    hero.fold[Try[Character]](Failure(new NoSuchElementException("No hero")))(Success(_))
  }
